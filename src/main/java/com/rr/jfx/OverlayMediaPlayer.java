 ...
 * <A class=referer href="http://my.oschina.net/u/244147" target=_blank>@see</A>
...
public class OverlayMediaPlayer extends Application {
	...
	private static final String MEDIA_URL = "z:/test.mp4";
	...
	private void init(Stage primaryStage) {
		...
		mediaPlayer = new MediaPlayer(new Media(file.getPath()));
//		mediaPlayer = new MediaPlayer(new Media(MEDIA_URL));
	...
	static class PlayerPane extends BorderPane {
		...
		public PlayerPane(final MediaPlayer mp) {
			...
			setCenter(mvPane);
			mediaTopBar = HBoxBuilder.create().padding(new Insets(5, 10, 5, 10)).alignment(Pos.CENTER).opacity(0).build();
			BorderPane.setAlignment(mediaTopBar, Pos.CENTER);
			mediaBottomBar = HBoxBuilder.create().padding(new Insets(5, 10, 5, 10)).alignment(Pos.CENTER).opacity(0).build();
			...
			setOnMouseEntered(new EventHandler<MouseEvent>() {
				...
				public void handle(MouseEvent t) {
					...
					transition = ParallelTransitionBuilder.create().children(FadeTransitionBuilder.create().node(mediaTopBar).toValue(1).duration(Duration.millis(200)).interpolator(Interpolator.EASE_OUT).build(), FadeTransitionBuilder.create().node(mediaBottomBar).toValue(1).duration(Duration.millis(200)).interpolator(Interpolator.EASE_OUT).build()).build();
					transition.play();
				}
			});
			setOnMouseExited(new EventHandler<MouseEvent>() {
				...
				public void handle(MouseEvent t) {
					...
					transition = ParallelTransitionBuilder.create().children(FadeTransitionBuilder.create().node(mediaTopBar).toValue(0).duration(Duration.millis(800)).interpolator(Interpolator.EASE_OUT).build(), FadeTransitionBuilder.create().node(mediaBottomBar).toValue(0).duration(Duration.millis(800)).interpolator(Interpolator.EASE_OUT).build()).build();
					transition.play();
				}
			});
			...
			// Time label             
			Label timeLabel = LabelBuilder.create().text("Time").minWidth(Control.USE_PREF_SIZE).textFill(Color.WHITE).build();
			...
			volumeSlider = SliderBuilder.create()
					.id("media-slider").prefWidth(120).maxWidth(Region.USE_PREF_SIZE).minWidth(30).build();
			...
			mediaBottomBar = HBoxBuilder.create().id("bottom").spacing(0).alignment(Pos.CENTER).children(ButtonBuilder.create().id("back-button").text("Back").onAction(backAction).build(), ButtonBuilder.create().id("stop-button").text("Stop").onAction(stopAction).build(), ButtonBuilder.create().id("play-button").text("Play").onAction(playAction).build(), ButtonBuilder.create().id("pause-button").text("Pause").onAction(pauseAction).build(), ButtonBuilder.create().id("forward-button").text("Forward").onAction(forwardAction).build()).build();

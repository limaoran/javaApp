 ...
 * 更改文件的访问权限以类似的方式为 chmod(1) 程序的示例代码。
...
public class Chmod {
    ...
    public static Changer compile(String exprs) {
        ...
        for (String expr : exprs.split(",")) {
            ...
            if(add){
                if(u){
                    if(r) toAdd.add(PosixFilePermission.OWNER_READ);
                    if(w) toAdd.add(PosixFilePermission.OWNER_WRITE);
                    if(x) toAdd.add(PosixFilePermission.OWNER_EXECUTE);
                }
                if(g){
                    if(r) toAdd.add(PosixFilePermission.GROUP_READ);
                    if(w) toAdd.add(PosixFilePermission.GROUP_WRITE);
                    if(x) toAdd.add(PosixFilePermission.GROUP_EXECUTE);
                }
                if(o){
                    if(r) toAdd.add(PosixFilePermission.OTHERS_READ);
                    if(w) toAdd.add(PosixFilePermission.OTHERS_WRITE);
                    if(x) toAdd.add(PosixFilePermission.OTHERS_EXECUTE);
                }
            }
            if(remove){
                if(u){
                    if(r) toRemove.add(PosixFilePermission.OWNER_READ);
                    if(w) toRemove.add(PosixFilePermission.OWNER_WRITE);
                    if(x) toRemove.add(PosixFilePermission.OWNER_EXECUTE);
                }
                if(g){
                    if(r) toRemove.add(PosixFilePermission.GROUP_READ);
                    if(w) toRemove.add(PosixFilePermission.GROUP_WRITE);
                    if(x) toRemove.add(PosixFilePermission.GROUP_EXECUTE);
                }
                if (o) {
                    if (r) toRemove.add(PosixFilePermission.OTHERS_READ);
                    if (w) toRemove.add(PosixFilePermission.OTHERS_WRITE);
                    if (x) toRemove.add(PosixFilePermission.OTHERS_EXECUTE);
                }
            }
            if (assign) {
                if (u) {
                    if (r) toAdd.add(PosixFilePermission.OWNER_READ);
                    else toRemove.add(PosixFilePermission.OWNER_READ);
                    if (w) toAdd.add(PosixFilePermission.OWNER_WRITE);
                    else toRemove.add(PosixFilePermission.OWNER_WRITE);
                    if (x) toAdd.add(PosixFilePermission.OWNER_EXECUTE);
                    else toRemove.add(PosixFilePermission.OWNER_EXECUTE);
                }
                if (g) {
                    if (r) toAdd.add(PosixFilePermission.GROUP_READ);
                    else toRemove.add(PosixFilePermission.GROUP_READ);
                    if (w) toAdd.add(PosixFilePermission.GROUP_WRITE);
                    else toRemove.add(PosixFilePermission.GROUP_WRITE);
                    if (x) toAdd.add(PosixFilePermission.GROUP_EXECUTE);
                    else toRemove.add(PosixFilePermission.GROUP_EXECUTE);
                }
                if (o) {
                    if (r) toAdd.add(PosixFilePermission.OTHERS_READ);
                    else toRemove.add(PosixFilePermission.OTHERS_READ);
                    if (w) toAdd.add(PosixFilePermission.OTHERS_WRITE);
                    else toRemove.add(PosixFilePermission.OTHERS_WRITE);
                    if (x) toAdd.add(PosixFilePermission.OTHERS_EXECUTE);
                    else toRemove.add(PosixFilePermission.OTHERS_EXECUTE);
                }
            }
        }
